...
class RandomCropAndResizeCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    img_c = data_layout == DataLayout::kNCHW ? img->dims()[0] : img->dims()[2];
    std::vector<int64_t> out_shape = {static_cast<int64_t>(x->size()), img_c,
                                      size[0], size[1]};
    out->Resize(framework::make_ddim(out_shape));
    auto* out_tensor = &out->Slice(0, 1);
